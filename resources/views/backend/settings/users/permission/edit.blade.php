...
<div class="d-flex justify-content-between align-items-center mb-2">
    <!-- Elemen H1 -->
    <h1 class="h3 text-gray-800">{{ __('Tambah Permissions') }}</h1>
    
    <!-- Elemen A Href di Sebelah Kanan -->
    <a href="{{ route('settings.permissions') }}" class="btn btn-md btn-primary shadow-sm">
        ...
        <div class="card-body">
            <form action="{{ route('settings.permissions.update', $permission->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $permission->name) }}"> <!-- Pastikan name diisi dengan data dari $permission -->
                    @error('name')
